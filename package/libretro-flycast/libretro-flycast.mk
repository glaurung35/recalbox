...
LIBRETRO_FLYCAST_SUPP_OPT += CC_AS="$(TARGET_CC)" LDFLAGS=-lmali
...
LIBRETRO_FLYCAST_SUPP_OPT += CC_AS="$(TARGET_CC)"
else ifeq ($(BR2_PACKAGE_RECALBOX_TARGET_RPI3)$(BR2_PACKAGE_RECALBOX_TARGET_RPIZERO2),y)
LIBRETRO_FLYCAST_PLATFORM = rpi3-mesa
LIBRETRO_FLYCAST_SUPP_OPT += CC_AS="$(TARGET_CC)"
else ifeq ($(BR2_PACKAGE_RECALBOX_TARGET_RPI4_64),y)
LIBRETRO_FLYCAST_PLATFORM = rpi4_64
LIBRETRO_FLYCAST_SUPP_OPT += HAVE_LTCG=0 CC_AS="$(TARGET_CC)"
else
LIBRETRO_FLYCAST_PLATFORM = $(RETROARCH_LIBRETRO_PLATFORM)
LIBRETRO_FLYCAST_SUPP_OPT =
endif

LIBRETRO_FLYCAST_SUPP_OPT += ARCH=$(BR2_NORMALIZED_ARCH)

ifeq ($(BR2_PACKAGE_RECALBOX_HAS_LIBGLES),y)
LIBRETRO_FLYCAST_SUPP_OPT += FORCE_GLES=1
