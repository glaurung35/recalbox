...
function mlPair {
  ...
  if [ -z "$1" ]; then
    ...
    sed -E '/^certificate=/!d;s/.*\(([^)]+)\).*/\1/;s/\\n/\n/g' /recalbox/share/system/.config/Moonlight\ Game\ Streaming\ Project/Moonlight.conf >"$mlKeydir/client.pem"
    sed -E '/^key=/!d;s/.*\(([^)]+)\).*/\1/;s/\\n/\n/g' /recalbox/share/system/.config/Moonlight\ Game\ Streaming\ Project/Moonlight.conf >"$mlKeydir/key.pem"
    sed -n -E '/(hostname='"$mlHost"'|localaddress='"$mlIp"')/,/.*uuid=/!d;s/.*uuid=(.*)/\1/p' /recalbox/share/system/.config/Moonlight\ Game\ Streaming\ Project/Moonlight.conf >"$mlKeydir/uniqueid.dat"
...
#    sed -E '/uuid=/!d;s/.*uuid=//' /recalbox/share/system/.config/Moonlight\ Game\ Streaming\ Project/Moonlight.conf >"$mlKeydir/uniqueid.dat"
  fi
}
...
function mlScrape {
  ...
  if [[ -z $hostOrIp ]]; then
    longImgPath="${mlRomDir}/media/images"
    shortImgPath='./media/images'
  else
    longImgPath="${mlRomDir}/media/images/${hostOrIp}"
    shortImgPath="./media/images/${hostOrIp}"
  fi
