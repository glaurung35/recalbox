...
define MUPEN64PLUS_OMX_BUILD_CMDS
	...
	CFLAGS="$(TARGET_CFLAGS) $(COMPILER_COMMONS_CFLAGS_SO)" \
		...
		$(MAKE) CXX="$(TARGET_CXX)" CC="$(TARGET_CC)" LD="$(TARGET_LD)" RANLIB="$(TARGET_RANLIB)" AR="$(TARGET_AR)" CROSS_COMPILE="$(STAGING_DIR)/usr/bin/" \
			...
			-C $(@D)/projects/unix all $(MUPEN64PLUS_CORE_PARAMS) MACHINE=armv6l OPTFLAGS="$(TARGET_CXXFLAGS)"
endef
...
define MUPEN64PLUS_OMX_INSTALL_TARGET_CMDS
	...
	CFLAGS="$(TARGET_CFLAGS) $(COMPILER_COMMONS_CFLAGS_SO)" \
		...
		$(MAKE) CXX="$(TARGET_CXX)" CC="$(TARGET_CC)" LD="$(TARGET_LD)" RANLIB="$(TARGET_RANLIB)" AR="$(TARGET_AR)" CROSS_COMPILE="$(STAGING_DIR)/usr/bin/" \
			...
			INSTALL_STRIP_FLAG="" \
			-C $(@D)/projects/unix all $(MUPEN64PLUS_CORE_PARAMS) MACHINE=armv6l OPTFLAGS="$(TARGET_CXXFLAGS)" install
endef
