...
rb_wifi_configure() {
  ...
  settings_priority=$("$system_setting" -command load -key "wifi${X}.priority" -source "$config_file")
  ...
  if [[ "$settings_ssid" != "" ]] ;then
    ...
    [ -n "$settings_priority" ] && wpa_cli -i "$interface" set_network "$network" priority "$settings_priority" >/dev/null || exit 1
