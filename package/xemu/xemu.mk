...
define XEMU_INSTALL_TARGET_CMDS
	...
	# XEmu app data
	mkdir -p $(TARGET_DIR)/recalbox/share_init/xemu/data
	mkdir -p $(TARGET_DIR)/recalbox/share_upgrade/bios/xbox
	cp $(@D)/data/* $(TARGET_DIR)/recalbox/share_init/xemu/data/
	$(UNZIP) -ob $(XEMU_DL_DIR)/xbox_hdd.qcow2.zip xbox_hdd.qcow2 -d \
	    $(TARGET_DIR)/recalbox/share_init/xemu/data
	cp $(BR2_EXTERNAL_RECALBOX_PATH)/package/xemu/exit.p2k \
	    $(TARGET_DIR)/recalbox/share_upgrade/roms/xbox/.p2k.cfg
endef
