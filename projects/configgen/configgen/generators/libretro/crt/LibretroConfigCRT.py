...
class LibretroConfigCRT:
    ...
    def createConfigForMode(self, region: str, mode: Mode, viewport_width: int, viewport_height: int, game_is_tate: int, system_rotation: Rotation, multiplier: int, k31:bool=False) -> \
            typing.Dict[
                str, typing.Any]:
        ...
        if (system_rotation.value == Rotation.left or system_rotation.value == Rotation.right) and not k31:
            if game_is_tate:
                ...
                for region in extension:
                    final_width = mode.height * (3/4) * multiplier
                    final_height = mode.height
                    final_x = (mode.width - final_width) // 2
                    final_y = 0
                config["video_smooth"] = '"true"'
                    ...
                    for region in [CRTVideoStandard.PAL, CRTVideoStandard.NTSC]:
                        config.update(
                            self.createConfigForMode(region,
                                                     ...
                                                     gameIsTate,
                                                     system.Rotation,
                                                     6 if defaultMode == "1920@31KHz-double:all:240@120" else 2,
                                                     k31=True)
                        )
