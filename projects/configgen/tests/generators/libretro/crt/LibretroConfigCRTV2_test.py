...
def configureForCrt(emulator: Emulator, crtvideostandard="auto", crtresolutiontype="progressive", crtscreentype="15kHz",
                    ...
                    crt_verticaloffset_p320x240=0,crt_verticaloffset_p320x224=0, crt_viewportwidth_p320x224=0, rotation=0):
    emulator.configure(keyValueSettings(""),
                       ExtraArguments("", "", "", "", "", "", "", "", crtvideostandard, crtresolutiontype,
                                      ...
                                      crt_viewportwidth_p320x224=crt_viewportwidth_p320x224,
                                      crt_verticaloffset_p320x224=crt_verticaloffset_p320x224,
                                      rotation=rotation))
    return emulator
...
def test_given_mslug5_on_fbneo_then_return_a_mode_with_offset(mocker, system_fbneo):
    ...
    emulator = configureForCrt(system_fbneo, crtsuperrez="original", crt_verticaloffset_p320x224=1)
...
def test_given_mslug5_on_fbneo_with_left_rotation_then_return_rotated_left(mocker, system_fbneo):
    ...
    assert config_lines["custom_viewport_height_ntsc"] == 224
    assert config_lines["custom_viewport_x_ntsc"] == 68
...
# def test_given_not_found_game_on_fbneo_then_return_default_mode(mocker, system_fbneo):
#     givenThoseFiles(mocker, {
#         ARCADEV2_TXT: "",
#     })
#
...
def test_given_viewport_width_then_create_extra_margins_for_original_rez(mocker, system_fbneo):
    givenThoseFiles(mocker, {
        ARCADEV2_TXT: "mslug5,fbneo,304,224,60,H",
    })


    emulator = configureForCrt(system_fbneo, crtsuperrez="original", rotation=0, crt_viewportwidth_p320x224=-6)
    mode = CRTModeGenerator().generate(328, 224, 60, CRTMonitorRanges[CRTScreenType.k15], False)
    ...
    assert config_lines["custom_viewport_x_ntsc"] == 12
