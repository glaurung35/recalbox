...
void GuiSaveStates::initGridsNStuff()
{
  ...
  mGameName = std::make_shared<TextComponent>(mWindow, mGame.Name(), mMenuTheme->menuText.font,
                                              mMenuTheme->menuText.color, TextAlignment::Right);
  ...
  //col with mList
  mGridBody->setColWidthPerc(0, 0.35f);
  mGridBody->setColWidthPerc(1, 0.05f);
  mGridBody->setColWidthPerc(2, 0.9f);
  //cols with metadata
  //spacer row
...
void GuiSaveStates::PopulateGrid()
{
  ...
  ed = std::make_shared<TextComponent>(mWindow, _("none"),
...
void GuiSaveStates::updateInformations(int i)
{
  ...
  if (i == 0)
  {
    mCurrentState = *new SaveState(Path::Empty);
    mModificationTimeField->setText(_("LAUNCH GAME FROM STATE"));
    return;
  }
  ...
  mThumbnail->setSize(mGridBody->getColWidth(2) * .6, mGridBody->getColWidth(2) * .6 * 3 / 4);
