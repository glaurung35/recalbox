...
void GuiMenuAdvancedSettings::SubMenuSelected(int id)
{
  switch ((Components)id)
  {
    ...
    case Components::CrtSubMenu: mWindow.pushGui(new GuiMenuCRT(mWindow, mSystemManager, Board::Instance().CrtBoard().GetCrtAdapter() == CrtAdapterType::RGBJamma ? _("JAMMA SETTINGS") : _("CRT SETTINGS"))); break;
...
void GuiMenuAdvancedSettings::DoResetFactory()
{
  ...
  IniFile recalboxBoot(Path("/boot/recalbox-boot.conf"), "GuiMenuAdvancedSettings - Recalbox-boot", false, true);
  // Reset rotation
