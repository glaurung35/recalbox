  ...
  : GuiMenuBase(window, _("CRT SETTINGS"), this)
{
  ...
  mEsResolution = AddList<std::string>(_("MENU RESOLUTION"), (int)Components::EsResolution, this, GetEsResolutionEntries(is31kHz && !multisync, supports120Hz, multisync), _(MENUMESSAGE_ADVANCED_CRT_ES_RESOLUTION_HELP_MSG));
...
std::string GuiMenuCRT::GetHorizontalFrequency()
{
  ...
  if (Board::Instance().CrtBoard().Has31KhzSupport() && !Board::Instance().CrtBoard().MultiSyncEnabled())
    result.append(1, ' ').append(_("(Hardware managed)"));
...
std::vector<GuiMenuBase::ListEntry<std::string>> GuiMenuCRT::GetEsResolutionEntries(bool only31kHz, bool supports120Hz, bool multisync)
{
