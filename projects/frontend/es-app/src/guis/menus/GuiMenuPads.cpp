  ...
  , mRefreshing(false)
{
  ...
  // Rumble
  AddSwitch(_("RUMBLE"), RecalboxConf::Instance().GetGlobalRumble(), (int)Components::Rumble, this, _(MENUMESSAGE_CONTROLLER_RUMBLE_HELP_MSG));
...
{
  mRefreshing = true;
  // Finally fill in all components
...
void GuiMenuPads::SwitchComponentChanged(int id, bool& status)
{
  ...
  if ((Components)id == Components::Rumble)
    RecalboxConf::Instance().SetGlobalRumble(status).Save();
}
