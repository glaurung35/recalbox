  ...
  , mSystemManager(systemManager)
{
  ...
  mType = AddList<String>(_("SCREENSAVER BEHAVIOR"), (int)Components::Type, this, GetTypeEntries(), _(MENUMESSAGE_UI_SCREENSAVER_BEHAVIOR_HELP_MSG));

  // add systems
  mSystemList = AddMultiList(_("SYSTEMS TO SHOW IN DEMO"), (int)Components::SystemList, this, GetSystemEntries());
}

std::vector<GuiMenuBase::ListEntry<String>> GuiMenuScreensavers::GetTypeEntries()
{
  std::vector<ListEntry<String>> list;

  String type = RecalboxConf::Instance().GetScreenSaverType();
...
void GuiMenuScreensavers::OptionListComponentChanged(int id, int index, const String& value)
{
  (void)index;
