    ...
    public:
      void Parse(FileData& game) override
      {
        if (game.IsGame())
        {
          ...
          std::string diskNumber = FileData::ExtractGameSupportNumber(fileName);
          std::string gameNameWithRegion = Strings::RemoveParenthesis(fileName).append(Regions::Serialize4Regions(Regions::ExtractRegionsFromNoIntroName(fileName))).append(diskNumber);
  ...
  for (const SystemData* system: searchableSystems)
  {
    for (auto& game: system->getGames()) {
      if(game->IsGame())
      {
        ...
        switch (context)
        {
          case FolderData::FastSearchContext::Family:
            for (auto& familly : game->Metadata().FamiliesAsList())
            {
              if (familly == originaltext)
        ...
        if (FolderData::FastSearchContext::Family != context && Strings::ToLowerUTF8(found) == lowercaseText)
          resultsList.push_back(game);
      }
    ...
    for(int i = (int)resultIndexes.Count(); --i >= 0; )
      if (mFastSearchSeries[resultIndexes[i].Context].Empty())
      {
        ...
        switch((FolderData::FastSearchContext)resultIndexes[i].Context)
        {
          ...
          case FolderData::FastSearchContext::Family: count = MetadataDescriptor::FamiliesIndexCount(); break;
