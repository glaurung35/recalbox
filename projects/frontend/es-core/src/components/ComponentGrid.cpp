                             ...
                             bool resize, const Vector2i& size, Borders border, UpdateType updateType)
{
  ...
  GridEntry entry(pos, size, comp, ha, va, canFocus, resize, updateType, border);
  mCells.push_back(entry);
...
void ComponentGrid::updateCellComponent(const GridEntry& cell)
{
  ...
  switch(cell.hzAlign)
  {
    case HAlignment::Left:break;
    case HAlignment::Center: pos[0] = pos.x() + (size.x() - cell.component->getSize().x()) / 2; break;
    case HAlignment::Right: pos[0] = pos.x() + (size.x() - cell.component->getSize().x()); break;
  }
  switch(cell.vtAlign)
  {
    case VAlignment::Top:break;
    case VAlignment::Center: pos[1] = pos.y() + (size.y() - cell.component->getSize().y()) / 2; break;
    case VAlignment::Bottom: pos[1] = pos.y() + (size.y() - cell.component->getSize().y()); break;
  }
