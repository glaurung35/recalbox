...
const ScrollTier QUICK_SCROLL_TIERS[] = {{500,  500},
                                         {5000, 114},
                                         {0,    8}};
...
class IList : public Gui
{
  ...
  public:
    ...
    inline const String& getSelectedName()
    {
      assert(size() > 0);
      return mEntries[mCursor].name;
    }

    inline const String& getNameAt(int index)
    {
      assert(size() > 0);
    ...
    [[nodiscard]] inline int size() const { return mEntries.size(); }

    [[nodiscard]] inline bool isEmpty() const { return mEntries.empty(); }

    [[nodiscard]] inline int getCursor() const { return mCursor; }
