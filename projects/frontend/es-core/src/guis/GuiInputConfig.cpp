...
void GuiInputConfig::addFormInput(const char* name, const String& label, const char* icon, bool skippable, InputEvent::EventType preferredType)
{
...
void GuiInputConfig::setText(const String& msg, unsigned int color) {
...
bool GuiInputConfig::assign(InputEvent input)
{
  ...
  if (String("HotKey") != formInput.name &&
