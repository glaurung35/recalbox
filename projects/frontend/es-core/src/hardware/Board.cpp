...
BoardType Board::GetBoardType()
{
  ...
  String::List cpuInfo = Files::LoadAllFileLines(Path("/proc/cpuinfo"/* "/tmp/cpuinfo"*/));
  ...
  for(const String& line : cpuInfo)
    if (line.Extract(':', left, right, true))
    {
      ...
      if (left == MODEL_STRING && hardware.empty()) hardware = right;
    }
  // Identification data
  for(const String& line : cpuInfo)
    if (line.Extract(':', left, right, true))
      if (left == REVISION_STRING)
        revision = right;
  ...
  {
    if (hardware.LowerCase().Contains("raspberry"))
