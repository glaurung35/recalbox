<template>
  <div class="row roms">
    <div class="col col-xs-12 col-sm-12 col-md-12 table-container">
      <q-table
        :columns="roms.columns"
        :data="roms.data"
        :filter="roms.filter"
        :pagination.sync="roms.pagination"
        binary-state-sort
        dense
        flat
        row-key="name"
...
<script>
  ...
  export default {
    name: 'EmulationRomsTabRoms',
    created() {
      this.$store.dispatch('roms/getBySystem', this.$route.params.system)
    },
    methods: {
      openConfirm(slug) {
        ...
        this.$root.$emit(
          ...
          this.$t('emulation.roms.dialogs.remove.text'),
        )
      },
      remove() {
        this.$store.dispatch('roms/remove', this.selectedSlug)
        ...
        roms: {
          ...
          columns: [
            ...
            {name: 'genre', align: 'left', label: 'Genre', field: 'genre', sortable: true},
            {
              ...
              field: 'dateSortie',
              sortable: true,
              format: (val, row) => date.formatDate(date.extractDate(val, 'YYYY-MM-DD'), 'DD/MM/YYYY'),
            },
