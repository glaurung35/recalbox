...
uint8_t ssd1306_oled_write_line(uint8_t size, char* ptr)
{
    ...
    else if (size == SSD1306_FONT_PIXTYPE) // 8x8
    {
        font_table = (uint8_t*)Pixeltype16x16;
        font_table_width = 12;
        font_table_lines = 2;
    }
    else
    ...
    while(current_line < font_table_lines) {
        ...
        while (ptr[index] != 0 && i <= 1024)
        {
            ...
            uint8_t* font_ptr = &font_table[(ptr[index] - 0x20) * font_table_width*font_table_lines];
            uint8_t j = 0;
            for (j = current_line*font_table_width; j < (current_line+1)*font_table_width; j++)
            {
        ...
        current_line++;
        ssd1306_oled_set_XY(global_x, global_y+current_line);
    }
    return 0;
}
...
uint8_t ssd1306_oled_draw_hline(uint x, uint y, uint len)
{
    ssd1306_oled_set_XY(x, y/8);
    data_buf[0] = SSD1306_DATA_CONTROL_BYTE;
    memset(data_buf+1,0x01,len);
    /* for(index = x; index < x+len; index++){
        data_buf[i++] = 0xFF;
        data_buf[i++] = 0xFF;
        data_buf[i++] = 0x00;
        data_buf[i++] = 0x00;
    ...
    return _i2c_write(data_buf, len+1);
}

uint8_t ssd1306_oled_draw_vline(uint x, uint y, uint len)
{
    while(y/8 <8){
        ssd1306_oled_set_XY(x, y/8);
        data_buf[0] = SSD1306_DATA_CONTROL_BYTE;
