...
/**
 * save volume value to recalbox.conf
 */
void timer_callback(int signum) {
  char cmdline[255];
  snprintf(cmdline, sizeof(cmdline), "/usr/bin/recalbox_settings -command save -key audio.volume -value %ld", gVolume);
  syslog(LOG_INFO, "set volume to %ld in recalbox.conf\n", gVolume);
  system(cmdline);
}
 ...
 */
void read_recalbox_volume() {
  char cmdline[255];
  FILE *fp;
  char data[4];
  snprintf(cmdline, sizeof(cmdline), "/usr/bin/recalbox_settings -command load -key audio.volume -default %d", DEFAULT_VOLUME);
  ...
  pclose(fp);
  syslog(LOG_INFO, "initial volume set to %ld", gVolume);
}
...
// daemon control
  ...
  while (1) {
    ...
    wait_loop(pa_context_get_sink_info_by_name(context, (char *) default_sink_name, set_volume, &gVolume));
    alarm(1); // eventually save volume value in 1 second

    rd = read(fd, ev, sizeof(struct input_event) * 64);
    alarm(0); // disable saving volume value
    ...
    for (i = 0; i < rd / sizeof(struct input_event); i++) {
      ...
      if ((ev[i].type == EV_KEY) && (ev[i].code == KEY_VOLUMEUP) && (ev[i].value == 1) && (gVolume <= MAX_VOLUME - VOLUME_STEP))
        gVolume += VOLUME_STEP;
      if ((ev[i].type == EV_KEY) && (ev[i].code == KEY_VOLUMEDOWN) && (ev[i].value == 1) && (gVolume >= MIN_VOLUME + VOLUME_STEP))
        gVolume -= VOLUME_STEP;
    }
  }
}
